...
def face_preprocess(image, landmark_model_type = 'large', crop_size = 128):
    ...
    for i, face_landmarks_item in enumerate(face_landmarks_dict):
        ...
        cropped_face_item, left, top = corp_face(image_array = aligned_face, size = crop_size, landmarks = rotated_landmarks)
        # transfer landmarks to fit the cropped face
...
def save_image(imgpath, crop_size = 128):
    """ save images
    :param image_array: numpy array of a single image
    ...
    face, landmarks = face_preprocess(image = image_array, landmark_model_type = 'large', crop_size = crop_size)
    for i, face_item in enumerate(face):
        cv2.imwrite(str(i) + ".jpg", face_item)

if __name__=='__main__':

    # load image
    imgpath = 'C:\\Users\\ZHAO Yuzhi\\Desktop\\code\\face\\target_family\\taiwan family\\3.jpg'
    save_image(imgpath, 128)
